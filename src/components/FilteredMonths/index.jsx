...
const FilteredMonths = ({ purchases }) => {
	...
	useEffect(() => {
		if (selectedMonth) {
			fetch(
				`https://642ee23f2b883abc64198889.mockapi.io/purchases?${selectedMonth}`,
			)
	...
	for (let purchase of purchases) {
		...
		const purchaseMonth = purchase.date

		const amount = parseFloat(purchase.price)

		if (totalByMonth[purchaseMonth]) {

			totalByMonth[purchaseMonth] += amount
		} else {
